...
def open_warrior_editor():
    ...
    tk.Label(frame_left, text="Warriors",
             bg=BG_LIST, fg=TEXT,
             font=("Segoe UI", 12, "bold")).pack(pady=10)

    search_var = tk.StringVar()
    search_entry = tk.Entry(frame_left, textvariable=search_var,
                            bg="#2a2a2a", fg=TEXT, insertbackground="white")
    ...
    def single_field(parent, label):
        ...
        tk.Label(row, text=label, width=14,
                 bg=BG_PANEL, fg=TEXT).pack(side="left")
        entry = tk.Entry(row)
    ...
    def two_fields(parent, label1, label2):
        row = tk.Frame(parent, bg=BG_PANEL)
        row.pack(fill="x", pady=2)
        tk.Label(row, text=label1, width=10,
                 bg=BG_PANEL, fg=TEXT).pack(side="left")
        e1 = tk.Entry(row, width=8)
        e1.pack(side="left", padx=5)
        tk.Label(row, text=label2, width=10,
                 bg=BG_PANEL, fg=TEXT).pack(side="left")
    ...
    numeric_fields = [
        entry_base_hp, entry_base_attack, entry_base_defense, entry_base_speed,
        entry_hp_growth, entry_attack_growth, entry_defense_growth, entry_speed_growth,
        entry_skill1_id, entry_skill1_unlock,
        entry_skill2_id, entry_skill2_unlock,
        entry_skill3_id, entry_skill3_unlock
    ]
    ...
    def update_filtered_list():
        ...
        for w_id, w in warrior_cache.items():
            if query in w["name"].lower():
                ...
                warrior_listbox.insert(tk.END, f'{w_id} - {w["name"]}')
    ...
    def save_warrior():
        ...
        if not entry_name.get().strip():
            set_status("Nome obrigatório!", "red")
            return

        is_saving["value"] = True
        selected_id = current_selected_id["id"]

        data = {
            "id": selected_id or 0,
            "name": entry_name.get().strip(),
            "sprite": entry_sprite.get().strip(),
        }
        ...
        for field, key in [
            ...
            data[key] = int(field.get() or 0)

        from network.sender import send_save_warrior_template, request_warrior_list
        send_save_warrior_template(session.client_socket, data)
        request_warrior_list(session.client_socket)

        def reselect():
            update_filtered_list()
            if selected_id:
                for index, w_id in enumerate(filtered_ids):
                    if w_id == selected_id:
                        warrior_listbox.selection_set(index)
                        warrior_listbox.event_generate("<<ListboxSelect>>")
                        break
            is_saving["value"] = False
            set_status("Warrior salvo com sucesso!")

        editor.after(600, reselect)

    # ================= DELETE =================
    def delete_warrior():
        if not current_selected_id["id"]:
            set_status("Selecione um warrior para deletar.", "orange")
            return

        from network.sender import send_delete_warrior_template, request_warrior_list
        send_delete_warrior_template(session.client_socket, current_selected_id["id"])
        request_warrior_list(session.client_socket)

        clear_fields()
        set_status("Warrior deletado.", "orange")

    # ================= BUTTONS =================
    btn_frame = tk.Frame(frame_right, bg=BG_MAIN)
    btn_frame.pack(pady=15)

    tk.Button(btn_frame, text="Novo", width=12,
              command=clear_fields).pack(side="left", padx=5)

    tk.Button(btn_frame, text="Salvar", width=12,
              bg=ACCENT, fg="white",
              command=save_warrior).pack(side="left", padx=5)

    tk.Button(btn_frame, text="Delete", width=12,
              bg="#aa2222", fg="white",
              command=delete_warrior).pack(side="left", padx=5)

    # ================= REFRESH =================
    def refresh_list():
        if session.warrior_templates:
            update_filtered_list()
        editor.after(500, refresh_list)

    refresh_list()

    from network.sender import request_warrior_list
    request_warrior_list(session.client_socket)
# ================= UPDATE LIST =================
...
def update_warrior_list(warriors):
    ...
    for w in warriors:
        ...
        warrior_listbox.insert(tk.END, f'{w["id"]} - {w["name"]}')